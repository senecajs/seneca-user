...
module.exports = function user(options) {
  ...
  var user_ent  = seneca.make(options.user.zone,options.user.base,options.user.name)
  var login_ent = seneca.make(options.login.zone,options.login.base,options.login.name)
  ...
  seneca.add({init:name},function(args,done){
    this.act('role:util, cmd:define_sys_entity', {list:[
      { entity:user_ent.entity$, fields:options.user.fields},
      { entity:login_ent.entity$,fields:options.login.fields}
    ]},done)
  })
