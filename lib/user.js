...
module.exports = function user(options) {
  ...
  seneca.add( {role:name, cmd:'register'},
              {nick:'string$',email:'string$',username:'string$',atleastone$:['nick','email','username'],
                name:'string$',active:'boolean$',password:'string$',repeat:'string$' },
              cmd_register )    
  ...
  seneca.add( {role:name, cmd:'create-reset'},
              {nick:'string$',email:'string$',user:'object$',username:'string$',atleastone$:['nick','email','user','username'] },
              resolve_user(cmd_create_reset,false) )

  seneca.add( {role:name, cmd:'load-reset'},
              {token:'required$,string$'},
              cmd_load_reset )

  seneca.add( {role:name, cmd:'execute-reset'},
  ...
  function cmd_login(args,done){
    ...
    if( !user.active ) {
      seneca.log.debug('login/fail',why='not-active',q,user)
    ...
    if( args.auto ) {
      ...
      return hasher( args.password + user.salt, options.rounds, function(pass){
        if( user.pass !== pass ) {
    ...
    function make_login( user, why ) {
      ...
      var login = loginent.make$( seneca.util.deepextend(
        {},
        cleanargs,
        {
          id$     : uuid(),
    ...
    user.load$(q,function(err,user){
      ...
      if( pwd ) {
        if( pwd === pwd2 && 1 < pwd.length) {
          ...
          done({err:'user/password_mismatch'})
        }
      }
      else {
        updateuser(user)
