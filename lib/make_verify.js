...
module.exports = ctx => {
  ...
  return async function make_verify(msg) {
    ...
    var t_expiry = null != msg.expire_point ? msg.expire_point :
        null != msg.expire_duration ? t_c + msg.expire_duration :
    ...
    if(unique) {
      ...
      var existing_verify = await seneca.entity(ctx.sys_verify).load$(unique_query)

      if(existing_verify) {
        return {ok:false, why:'not-unique', details:{
          query: unique_query
        }}
      }
    }
